...
export const ElectionMapPrev2013 = ({ mapRef, electionData, setSelectedCandidates }) => {
  ...
  useEffect(() => {
    ...
    d3.json('data/41stCA2011/riding.geojson').then(geo => {
      const features = geo.features;
      mapRef.current = {
        map,
        zoomToRiding: (districtNumber) => {
          const match = features.find(f => f.properties["FEDUID"] == districtNumber);
          if (match) {
      ...
      const ridings = g.selectAll('path')
        ...
        .attr('fill', d => {
          const code = d.properties["FEDUID"];
      ...
      ridings.on('mouseover', function (event, d) {
        ...
        const selectedRidingCandidates = candidatesByRiding.find(r => r.key == d.properties["FEDUID"]);
      ...
      function handleRidingClick(d, event = null) {
        ...
        if (selectedRidingRef.current) {
          const prevCode = selectedRidingRef.current.properties["FEDUID"];
          g.selectAll('path')
            .filter(prevD => prevD.properties["FEDUID"] === prevCode)
  ...
  }, [electionData, setSelectedCandidates, mapRef]);
